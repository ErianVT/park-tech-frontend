...
export function PricesTable({ data, isLoading, onEditSucess }: PricesTableProps) {
            ...
            <TableHeader>
                <TableRow className="bg-muted">
                    {columns.map((item, index) => (
                        <TableHead key={index} className="text-muted-foreground text-[12px] text-left font-semibold uppercase px-4 py-3">
                            {item}
                        </TableHead>
                        ...
                        <tr key={i} className="border-t border-input">
                            {[...Array(5)].map((_, j) => (
                                <td key={j} className="px-4 py-4">
                                    <Skeleton
                                        className="h-4 rounded bg-input"
                ...
                {!isLoading && data.map((price) => {
                    ...
                    return (
                        <TableRow key={price.id} className="[&_td]:p-4">
                            ...
                            <TableCell >
                                <div className={cn("w-fit text-[12px] font-medium bg-gray-100/80 text-gray-600 rounded-lg px-2 py-1",
                            ...
                            <TableCell>
                                <div className="space-x-2">
                                    <EditPriceDialog editingPrice={price} onSuccess={onEditSucess} />
                                </div>
                            </TableCell>
                        </TableRow>
                    )
